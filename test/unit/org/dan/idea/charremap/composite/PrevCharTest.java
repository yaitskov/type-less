...
public class PrevCharTest {
    ...
    public void zeroOffset() {
        ...
        assertFalse(m.test(new ParentMatcherState("a", 0, seqUp(WHITE_SPACE))));
    }

    @Test
    public void offset1PredicateTrue() {
        Matcher m = prevChar(c -> c == 'a');
        assertTrue(m.test(new ParentMatcherState("ab", 1, seqUp(WHITE_SPACE))));
    }

    @Test
    public void offset1PredicateFalse() {
        Matcher m = prevChar(c -> c == 'b');
        assertFalse(m.test(new ParentMatcherState("ab", 1, seqUp(WHITE_SPACE))));
    }
}
